...
By "Just Obsidian," I mean using [Obsidian](Obsidian) for as much as possible.
- Tasks
- Projects
- Time logging
- Notes
...
- No friction
	- Time logging is done in bullet points
		- A summary is generated by dataview like [time-tracking-views](<../../archive/views/time-tracking-views.md>)
	- Tasks are done with the [tasks](tasks) plugin 
...
- Scheduling tasks is possible with the [day planner](day%20planner) plugin, but
...
- Calendars are universal, dumb pieces of technology. 
	...
	- Various third party clients like [Fantastical, Busycal](<../updates/busycal-vs-fantastical.md>), Calendar 366, even [Noteplan](Noteplan), etc.
...
- There's a lot of friction.
	- Switching between apps is as expensive as changing contexts. 
		- Even with iOS shortcuts, it takes a long time to link between calendar events, reminders and notes, which gets in the way of vocalising my thoughts. This is also why I considered [Apple Notes](Apple%20Notes) for a while today, but it solves one problem while introducing 10 others, like links breaking if you move notes around, no version control, random overheating reported on iPadOS on Reddit with many notes, etc. As much as [Forever Notes](https://www.youtube.com/@myforevernotes)
- [Reminders](Reminders) lacks a "scheduled" date and has a very strict subset of capabilities that obsidian tasks do.
- [Fantastical](Fantastical) and [Reminders](Reminders) lack a "share URL to event" action that would let me easily link tasks and events to notes. Instead, I have to rely on lots of Javascript using [Scriptable](Scriptable) on iOS to get the job done, which, again, takes time.
- [Fantastical](Fantastical) would need another tool like [Calflow](https://calflow.app) to provide statistics on time spent.
