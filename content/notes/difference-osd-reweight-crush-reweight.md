...
[./ceph](<./ceph.md>) has two ways of changing an OSD's weight.
...
- Ranges from 0 - 1, 0 being the equivalent of marked out and 1 is the equivalent of being marked in. If an OSD has been marked in, it'll have the `reweight` value of 1. We can confirm this with a small experiment.
	...
	- In the OSD, check osd reweight
	  ...
	  ```
- It forces CRUSH to move (`1 - reweight`) times the data that would have otherwise lived on this drive. However it does not change the weight of the host and therefore only causes data movement within the host, not across the crush map. This might cause a [./nearfull-osd](<./nearfull-osd.md>) situation as more data is allocated to a single OSD.
