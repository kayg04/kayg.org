...
In my case, it isn't critical when masking the `systemd-logind` service. However, if you use LXCs for a number of other processes where security is very important, consider enabling the `nesting` option in Proxmox instead. This would allow [systemd-logind](systemd-logind) to impose proper isolation and access control.

## Source

https://old.reddit.com/r/Proxmox/comments/ph10mb/comment/hbgmgfc